...
class Scraper:
    ...
    def amazonList(query):
        ...
        for item in items.find_all("div", {"class":"sg-col-20-of-24 s-result-item sg-col-0-of-12 sg-col-28-of-32 sg-col-16-of-20 sg-col sg-col-32-of-36 sg-col-12-of-16 sg-col-24-of-28"}):
            ...
            prdList.append(Scraper.prdTmplt(imgsrc, title, price, newlink))
    ...
    def ebayList(query):
        ...
        for item in items.find_all("div", {"class":"s-item__wrapper"}):
            ...
            prdList.append(Scraper.prdTmplt(img, title, price, newlink))
                    
        allProduct = prdList
        return allProduct  
           
    def neweggList(query):
        headers = {"User-agent":"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36"}
    ...
    def jetList(query):
        ...
        for item in items.find_all("div", {"class":"fSSCaC"}):
            ...
            prdList.append(Scraper.prdTmplt('https://images.unsplash.com/photo-1568650620424-23336b69ba8b?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=750&q=80', title, price, link)) 
